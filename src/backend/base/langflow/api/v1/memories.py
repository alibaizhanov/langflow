...
async def list_memory_bases(
    ...
    async with session_scope() as db:
        stmt = _service.list_for_user_stmt(user_id=current_user.id, flow_id=flow_id)
...
async def list_sessions(
    ...
    async with session_scope() as db:
        try:
            mb = await _service._get_mb_or_raise(db, memory_base_id, current_user.id)  # noqa: SLF001
        except ValueError as exc:
        ...
        for s in raw_page.items:
            pending_count = await _service._count_pending(db, mb, s)  # noqa: SLF001
